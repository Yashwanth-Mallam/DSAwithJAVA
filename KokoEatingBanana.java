// Problem Statement: A monkey is given ‘n’ piles of bananas, whereas the 'ith' pile has ‘a[i]’ bananas.
// An integer ‘h’ is also given, which denotes the time (in hours) for all the bananas to be eaten.
// Each hour, the monkey chooses a non-empty pile of bananas and eats ‘k’ bananas. If the pile contains less than ‘k’ bananas,
// then the monkey consumes all the bananas and won’t eat any more bananas in that hour.
// Find the minimum number of bananas ‘k’ to eat per hour so that the monkey can eat all the bananas within ‘h’ hours.

// Example 1:
// Input Format: N = 4, a[] = {7, 15, 6, 3}, h = 8
// Result: 5
// Explanation: If Koko eats 5 bananas/hr, he will take 2, 3, 2, and 1 hour to eat the piles accordingly. 
// So, he will take 8 hours to complete all the piles.  

// Example 2:
// Input Format: N = 5, a[] = {25, 12, 8, 14, 19}, h = 5
// Result: 25
// Explanation: If Koko eats 25 bananas/hr, he will take 1, 1, 1, 1, and 1 hour to eat the piles accordingly. 
// So, he will take 5 hours to complete all the piles.
// Before moving on to the solution, let’s understand how Koko will eat the bananas. Assume,
// the given array is {3, 6, 7, 11} and the given time i.e. h is 8. 
// First of all, Koko cannot eat bananas from different piles. He should complete the pile he has chosen and then he can go for another pile.
// Now, Koko decides to eat 2 bananas/hour. So, in order to complete the first he will take
// 3 / 2 = 2 hours. Though mathematically, he should take 1.5 hrs but it is clearly stated in the question that after completing a 
// pile Koko will not consume more bananas in that hour. So, for the first pile, Koko will eat 2 bananas in the first hour and then 
// he will consume 1 banana in another hour. 
// From here we can conclude that we have to take ceil of (3/2). Similarly, we will calculate the times for other piles.

// 1st pile: ceil(3/2) = 2 hrs
// 2nd pile: ceil(6/2) = 3 hrs
// 3rd pile: ceil(7/2) = 4 hrs
// 4th pile: ceil(11/2) = 6 hrs

// Koko will take 15 hrs in total to consume all the bananas from all the piles. 
// Observation: Upon observation, it becomes evident that the maximum number of bananas (represented by 'k') that Koko
// can consume in an hour is obtained from the pile that contains the largest quantity of bananas.
// Therefore, the maximum value of 'k' corresponds to the maximum element present in the given array.
// So, our answer i.e. the minimum value of ‘k’ lies between 1 and the maximum element in the array i.e. max(a[]).
// Now, let’s move on to the solution.

import java.util.Scanner;

public class KokoEatingBanana {
    //Method to find the max number of bananas koko can eat.
    public static int findmax(int[] arr){
        int maxpiles = Integer.MIN_VALUE;
        for(int i=0; i<arr.length;i++){
            maxpiles = Math.max(maxpiles, arr[i]);
        }
        // lets return the max number of bananas in the piles, a[] = {7, 15, 6, 3} while is 15.
        return maxpiles;
    }
    
    // Method to find the total time requreid to eat number of bananas in the pile.
    public static int TotalTimeFinder(int[] arr,int perhour){
        int totalTime = 0;
        for(int i=0;i<arr.length;i++){
            totalTime += Math.ceil((double)(arr[i]) / (double)(perhour));
        }
        return totalTime;
    }

    // Method to find the maxx bananas can koko eat.
    // to find that we need to use BINARY SEARCH ALGO. (to decrese the time complexity.)
    public static int BananasCanBeEatenPerHour( int[] arr,int hours){
        int low = 0;
        int high = findmax(arr); // this will give us the max ele in the arr.
        
        int ans = high;

        while(low <= high){
            int mid = low + (high - low) / 2;
            int totalTime = TotalTimeFinder(arr, mid);
            if(totalTime <= hours){
                ans = mid;
                high = mid - 1;
            } else {
                low = mid + 1;
            }
        }
        return ans;
    }
    // main
    public static void main(String[] args) {
        Scanner sc = new Scanner(System.in);
        System.out.println("enter the hours: ");
        int hours = sc.nextInt();
        System.out.println("enter the size of arr: ");
        int n = sc.nextInt();
        int[] arr = new int[n];
        System.out.println("enter the number of Piles(arr): ");
        for(int i=0; i<n; i++){
            arr[i] = sc.nextInt();
        }
        int ans = BananasCanBeEatenPerHour(arr, hours);
        System.out.println("Koko should eat at least " + ans + " bananas/hr.");
        sc.close();
    }
}
